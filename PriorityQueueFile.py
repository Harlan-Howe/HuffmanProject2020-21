...
class PriorityQueue (Generic[T]):
    ...
    def is_a_heap(self)->bool:
        ...
        while self.in_bounds(index):
            ...
            this = self.node_at_index(index)
            if self.has_left_child(index):
                left_child = self.node_at_index(self.left_child_of_index(index))
                if self.a_has_priority_over_b(left_child,this):
                    return False
            if self.has_right_child(index):
                right_child = self.node_at_index(self.right_child_of_index(index))
                if self.a_has_priority_over_b(right_child,this):
                    return False
            # ----------------------
            index += 1
    ...
    def heapify_up(self, index:int):
        ...
        if index == 0:
            return
        this_node = self.node_at_index(index)
        parent_index = self.parent_of_index(index)
        parent_node = self.node_at_index(parent_index)
        if self.a_has_priority_over_b(this_node,parent_node):
            self.set_node_at_index(this_node,parent_index)
        ...
        if self.in_bounds(left_index):
            ...
            if self.in_bounds(right_index):
                ...
                if self.a_has_priority_over_b(right_node,left_node):
                    better_child_index = right_index
            better_child_node = self.node_at_index(better_child_index)
            if self.a_has_priority_over_b(better_child_node,current_node):
                self.set_node_at_index(better_child_node,index)
                self.set_node_at_index(current_node,better_child_index)
                self.heapify_down(better_child_index)
